...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Return True if there is at least one empty position that is impossible to fill because the set of symbols has
        been completely used up by other positions in the same row, column, or subsquare.
        ...
        if "*" not in self._symbols:
            ...
            for position in range(len(self._symbols)):
                if self._symbols[position] == "*":
                    if self._symbol_set == ((self._row_set(position) | self._column_set(position) |
                                                     self._subsquare_set(position)) - {"*"}):
                        return True
            return False
